# mlp_from_excel_v1.py
# -----------------------------------------------------------
# MLP (Dense) forecast for "Target" from your Excel sheet.
# - Uses all columns except "date 2" and "Target" as features
# - Flattens a 'lookback' window to feed a 3-layer Dense network
...
from tensorflow.keras.layers import Dense, Dropout, Flatten # Changed LSTM to Flatten
...
MODEL_DIR  = Path("mlp_artifacts") # Changed from lstm_artifacts
...
# -------------------
# Model (CHANGED TO 3-LAYER DENSE/MLP)
# -------------------
tf.random.set_seed(42)

# Input shape is still the 3D sequence
input_shape = (LOOKBACK, Xtr.shape[-1])

model = Sequential([
    Input(shape=input_shape),
    
    # Flatten the (LOOKBACK, n_features) sequence into a single vector
    # Input shape becomes (LOOKBACK * n_features)
    Flatten(),
...
# Save artifacts
# -------------------
model_path = MODEL_DIR / "mlp_target_forecaster.keras" # Changed filename
